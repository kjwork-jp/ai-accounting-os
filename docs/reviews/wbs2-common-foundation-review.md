...
## 9. デプロイ時の差分不一致トラブルシュート（追記）

Vercel で `Toaster` 重複宣言エラーが発生した場合、ローカルとデプロイ対象コミットが一致していない可能性がある。

- Vercel のデプロイ詳細で `Source` の commit SHA が期待値か確認する
- `Redeploy without cache` でキャッシュを無効化して再ビルドする
- 対象ブランチが `main` 以外になっていないか確認する
- ローカルで `pnpm build` が通ることを確認したうえで再デプロイする
